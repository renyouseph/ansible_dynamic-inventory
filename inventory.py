...
if SCAN_ALL_REGION:
    if ACCESS_KEY and SECRET_KEY:
        scan_all_client = boto3.client('ec2',aws_access_key_id=ACCESS_KEY,aws_secret_accesse_key=SECRET_KEY)
    else:
        scan_all_client = boto3.client('ec2')

    REGIONS = [ region['RegionName'] for region in scan_all_client.describe_regions()['Regions'] ]

for region in REGIONS:
    if ACCESS_KEY and SECRET_KEY:
        ec2_client = boto2.resource('ec2',region_name=region,aws_access_key_id=ACCESS_KEY,aws_secret_access_key=SECRET_KEY)
    else:
